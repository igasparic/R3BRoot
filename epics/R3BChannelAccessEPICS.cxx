...
double R3BChannelAccessEPICS::Get()
{
  return fValue;
}

double *R3BChannelAccessEPICS::GetPointer()
{
  return &fValue;
}

chid R3BChannelAccessEPICS::GetID() const
{
  return fId;
}

chtype R3BChannelAccessEPICS::GetType() const
{
  return fType;
}

void R3BChannelAccessEPICS::Set(double value)
{
  fValue = value;
}
...
R3BChannelAccess *R3BChannelAccessGroupEPICS::CreateChannel(TString const
    &name)
{
  fNeedSearch = true;
  return new (fChannelArray[fChannelArray.GetEntriesFast()])
      R3BChannelAccessEPICS(name);
}
...
bool R3BChannelAccessGroupEPICS::PendIO(char const *op)
{
  ...
  switch (status) {
    case ECA_NORMAL:
      return true;
    case ECA_TIMEOUT:
      LOG(ERROR) << "R3BChannelAccessGroupEPICS::Get : CA " << op <<
          " failed." << FairLogger::endl;
      return false;
    default:
      LOG(ERROR) << "R3BChannelAccessGroupEPICS::Get : Unexpected CA " << op
          << " error." << FairLogger::endl;
      return false;
  }
}
...
{
  return new (fGroupArray[fGroupArray.GetEntriesFast()])
