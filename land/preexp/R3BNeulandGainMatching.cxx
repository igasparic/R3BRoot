...
void R3BNeulandGainMatching::Exec(Option_t* option)
{
  ...
  if (checkiteration == fNofPMTs) finished=1;
    //std::cout << "Channels done: " << checkiteration << std::endl;
  
  if (nHits < 10) return; // better cosmic peaks
  ...
  for (Int_t i = 0; i < nHits; i++)
	  ...
	  iteration[iPlane][iBar][iSide]<MAX_STEPS)
	{
	  ...
	  if (e == xtarget - 10000) {
	    ...
	    std::cout << "p" << iPlane+1 << "b" << iBar+1 << "t" << iSide+1 << "  e: " << e << "  esum: " << esum[iPlane][iBar][iSide] << "  ealt: " << ealt[iPlane][iBar][iSide] << std::endl;

	    hv[iPlane][iBar][iSide] = hv[iPlane][iBar][iSide] + Kp*e + Ki*Ta*esum[iPlane][iBar][iSide] + Kd*(e-ealt[iPlane][iBar][iSide])/Ta;
	  ...
	  if(iteration[iPlane][iBar][iSide]==MAX_STEPS) {
		...
		std::cout << "Reached max steps but last check was already inside 5% accuracy. Taking last hv value." << std::endl;
		newhv = hv[iPlane][iBar][iSide];
	      } else {
...
Double_t R3BNeulandGainMatching::searchcosmicpeaknb(TH1 *hin, Double_t width) {
  ...
  for (Int_t p=0;p<nfound;p++) {
    std::cout << "Peak " << p << " at x=" << xpeaks[p] << std::endl; 
    if(LSEARCHNB<xpeaks[p] && xpeaks[p]<HSEARCHNB){ // ig 60<  <200
      //xp = xpeaks[p];
