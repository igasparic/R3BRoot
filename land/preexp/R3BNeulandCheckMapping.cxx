      ...
      for (Int_t bar = 0; bar < fPaddlesPerPlane; bar++)
	{
	  for (Int_t pmt = 0; pmt < 2; pmt++)
	      ...
	      for (;;)
		{
		  ...
		  if (entrycheck.vmon->Get() < 20.) break;
		  sleep(1);
		}
	    }
	}
...
std::cout << "\nAll off! Enabling the first tube...\n";
    ...
    for (;;) {
      entryon.group->Fetch();
      if (fabs(entryon.vmon->Get() - entryon.vtarget->Get()) < 15.) break;
      sleep(1);
    }
    timestamp0.Set();
...
void R3BNeulandCheckMapping::Exec(Option_t* option)
{
  ...
  std::cout << "Exec ts0=" << timestamp0 << " ts1=" << timestamp1 << " diff=" << difftime(timestamp1, timestamp0) << std::endl; 
