...
{   
    ...
    hToFvsBar = new TH2D("hTofvsBar", "Tof vs Bars", fNBars, 0.5, fNBars + 0.5, 5000, -240, 400);
    canvasHit->cd(3);
    hToFvsBar->Draw("colz");

    hToFcvsBar = new TH2D("hTofcvsBar", "Tofc vs Bars", fNBars, 0.5, fNBars + 0.5, 5000, -240, 400);
    canvasHit->cd(4);
    hToFcvsBar->Draw("colz");

    hToFvsX = new TH2D("hTofvsX", "Tof vs X", 300, -150., 150., 5000, -240, 400);
    canvasHit->cd(5);
    hToFvsX->Draw("colz");

    hTofvsEhit = new TH2D("hTofvsEhit", "Tof vs Ehit", 1000, 0, 60, 5000, -240, 400);
    canvasHit->cd(6);
    ...
    hHitEvsBarCosmics->Draw("colz");
     ...
     hTdiffvsBarCosmics = new TH2D("hTdiffvsBarCosmics", "Tdiff vs Bars cosmics", fNBars, 0.5, fNBars + 0.5, 1000, -60, 60);
    ...
    auto canvasPlaneXYTofcut = new TCanvas("NeulandPlaneXYTofcut", "NeulandPlaneXYTofcut", 10, 10, 850, 850);
    canvasPlaneXY->Divide(4, 4);
    for (unsigned int i = 0; i < fNPlanes; i++)
    {
        ahXYperPlaneTofcut[i] = new TH2D("hHitXYPlaneTofcut" + TString::Itoa(i, 10),
                                   "Hit XY Plane Tofcut" + TString::Itoa(i, 10),
                                   150,
                                   -150,
                                   150,
                                   150,
                                   -150,
                                   150);
        canvasPlaneXYTofcut->cd(i + 1);
        ahXYperPlaneTofcut[i]->Draw("colz");
    }
    canvasPlaneXYTofcut->cd(0);
    run->AddObject(canvasPlaneXYTofcut);
	...
	if (IsBeam())
	  {
	    ...
	    if (hit->GetQdc()>7.)
	      {
		hToFvsX->Fill(hit->GetXX()+(plane%2)*5.*(randx-0.5), fmod(hit->GetTdc() - start+5*8192+2000,5*2048));
		hToFvsBar->Fill(bar, fmod(hit->GetTdc() - start+5*8192+2000,5*2048));
		hToFcvsBar->Fill(bar, fmod(hit->GetTdc() - start+5*8192+2000,5*2048) - tof_corr);
		//hToFvsBar->Fill(bar, 1520./s*fmod(hit->GetTdc() - start+5*8192+2000,5*2048));
	      }
	    
	    hTofvsEhit->Fill(hit->GetQdc(), fmod(hit->GetTdc() - start+5*8192+2000,5*2048) - tof_corr);
	    //hTofvsEhit->Fill(hit->GetQdc(), 1520./s*fmod(hit->GetTdc() - start+5*8192+2000,5*2048));
	    
	    ahXYperPlane[plane]->Fill(hit->GetXX()+(plane%2)*5.*(randx-0.5),
				      hit->GetYY()+((plane+1)%2)*5.*(randx-0.5));
              if ((fmod(hit->GetTdc() - start+5*8192+2000,5*2048) - tof_corr)<80.&&(fmod(hit->GetTdc() - start+5*8192+2000,5*2048) - tof_corr)>0.)			      
		ahXYperPlaneTofcut[plane]->Fill(hit->GetXX()+(plane%2)*5.*(randx-0.5),
				      hit->GetYY()+((plane+1)%2)*5.*(randx-0.5));
				      }		      
