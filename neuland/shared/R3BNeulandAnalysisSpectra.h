#ifndef R3BROOT_R3BNEULANDANALYSISSPECTRA_H
#define R3BROOT_R3BNEULANDANALYSISSPECTRA_H

#include "FairTask.h"
#include "R3BPaddleTamexMappedData.h"
#include "R3BLosCalData.h"
#include "R3BNeulandCalData.h"
//#include "R3BNeulandHit.h"
#include "R3BLandDigi.h"
#include "TCAConnector.h"

class TCanvas;
class TH1D;
class TH2D;

class R3BNeulandAnalysisSpectra : public FairTask
{

  public:
    R3BNeulandAnalysisSpectra();

    InitStatus Init() override;
    void Exec(Option_t*) override;
    void FinishTask() override;

  private:
    static const unsigned int fNPlanes = 16;
    static const unsigned int fNBars = fNPlanes * 50;

    TCAInputConnector<R3BPaddleTamexMappedData> fNeulandMappedData;
    TCAInputConnector<R3BNeulandCalData> fNeulandCalData;
    //TCAInputConnector<R3BNeulandHit> fNeulandHits;
    TCAInputConnector<R3BLandDigi> fNeulandHits;
    TCAInputConnector<R3BLosCalData> fLosCalData;

    TH1D* hTstart;
    TH1D* hNstart;

    std::array<TH1D*, 4> ahMappedBar1;
    std::array<TH1D*, 4> ahMappedBar2;

    std::array<TH2D*, 2> ahCalTvsBar;
    std::array<TH2D*, 2> ahCalEvsBar;

    TH2D* hHitEvsBar;
    
    TH2D* hHitEvsBarCosmics;
    
    TH2D* hTdiffvsBar;
    TH2D* hToFvsBar;
    TH2D* hToFvsTdiff;
    TH2D* hToFrawvsBar;
    TH2D* hToFxvsBar;
    TH2D* hToFxxvsBar;
    TH2D* hToFcvsBar;
    TH2D* hToFvsX;
    TH2D* hTofvsEhit;

    TH2D* hTofvsPx;

    TH2D* hPyvsPx;
    TH2D* hPxvsPz;
    TH2D* hPyvsPz;
    TH2D* hPxvsPzint;
    TH2D* hPyvsPzint;

    TH2D* hPxvsBar;
    TH2D* hPyvsBar;
    TH2D* hPzintvsBar;

    TH2D* hVelvsBar;

    TH1D* hDV;
    
    TH2D* hTdiffvsBarCosmics;
    TH2D* hDT375;
    TH2D* hDT425;

    TH2D* hXY;
    TH2D* hXYGamma;
    TH2D* hXYCut;
    
    std::array<TH2D*, fNPlanes> ahXYperPlane;
    std::array<TH2D*, fNPlanes> ahXYperPlaneGamma;
    std::array<TH2D*, fNPlanes> ahXYperPlaneCut;

  private:
    bool IsBeam() const;
    double GetTstart() const;

    ClassDefOverride(R3BNeulandAnalysisSpectra, 0)
};

#endif // R3BROOT_R3BNEULANDANALYSISSPECTRA_H
